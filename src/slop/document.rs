...
use iced_x86::MemorySizeOptions::Default;
...
/// `RBX::Document`.
pub struct Document {
    pub marshaller: Option<Arc<FunctionMarshaller>>,
    pub game: Option<Arc<dyn Game>>,
    /// `startedSignal` subscribers (bool = isTeleport).
    pub started_handlers: Vec<Box<dyn Fn(bool) + Send + Sync>>,
}
...
impl Document {
    ...
    pub fn initialize(&mut self, _hwnd: usize, use_chat: bool) {
        let game = rbx::create_player_game(&rbx::base_url());
    ...
    pub fn start(
        ...
        self.execute_script(data, launch_mode, vr_device);
    }
    ...
    fn execute_script(&self, data: String, launch_mode: LaunchMode, vr_device: Option<&str>) {
